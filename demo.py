...
def image_process(image_path, imgH=32, imgW=100, keep_ratio=False, min_ratio=1):
  ...
  img.sub_(0.5).div_(0.5)  # 3 channels?
...
class DataInfo(object):
  ...
  def __init__(self, voc_type):
    ...
    self.voc = get_vocabulary(voc_type, EOS=self.EOS, PADDING=self.PADDING, UNKNOWN=self.UNKNOWN)  # list, length=lengtg+3
    self.char2id = dict(zip(self.voc, range(len(self.voc))))
    self.id2char = dict(zip(range(len(self.voc)), self.voc))

    self.rec_num_classes = len(self.voc)  # [char_sets + 3] eos, padding, unknown
